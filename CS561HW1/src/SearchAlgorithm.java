...
public class SearchAlgorithm {
	...
	public static void main(String[] args) throws IOException {
		File file = new File("/Users/laicunhao/eclipse-workspace/CS561HW1/src/test1.txt");
	...
	private static void AstartSearch(int[][] MAP, List<String> endLocs, int[] startPtr, int maxRockHeight, FileWriter fileWriter) throws IOException {
		...
		for(int goal = 0; goal < endLocs.size(); goal++) {
			...
			Node startNode = new Node(startPtr, 0 + findStraightLineDistance(startPtr, endPoint), null );
...
//				parentMap[curNode.loc[0]][curNode.loc[1]] = curNode.parent;
				...
				for(int[] dir : DIRECTION) {
					...
					if(isValidCell(x, y) && isValidClimb(MAP, x, y, curNode.loc, maxRockHeight) && !visited.contains(x + "," + y)) {
						...
						if(dir[0] == 0 || dir[1] == 0) {
							nextNode = new Node(new int[] {x, y}, 10 + heightDifferenceAndMud(MAP, x, y, curNode.loc) + curNode.cost, curNode );
						}
						else {
							nextNode = new Node(new int[] {x, y}, 14 + heightDifferenceAndMud(MAP, x, y, curNode.loc) + curNode.cost, curNode );
						}
			...
			if(!visited.contains(endLocs.get(goal))) {
				...
				writeOutPath(fileWriter, endGoalEntry.get(endLocs.get(goal)), startPtr);
			}
...
//			parentMap[curNode.loc[0]][curNode.loc[1]] = curNode.parent;
		...
		for(int i = 0; i < endLocs.size(); i++) {
			if(!visited.contains(endLocs.get(i))) {
				...
				writeOutPath(fileWriter, endGoalEntry.get(endLocs.get(i)), startPtr);
			}
